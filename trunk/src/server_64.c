	...
	for(j = 0; j < getSymtableSize(dir); j++) 
  {
        ...
        ((dir == 'r') && (clients[cId].rupdated[j]))) 
    {
      *ioPtr = j; //Save index
      ioPtr++;    //Move pointer
      //Copy timestamp
      ioPtr[0] = tempSymtable[j].timestamp->tv_sec;
      ioPtr[1] = tempSymtable[j].timestamp->tv_usec;
      ioPtr += 2;      
...
//    if (secureRecv(clients[clId].fd,(ioBuffer+1),sizeof(struct timeval) + tempSymtable[ioBuffer[0]].length * sizeof(int32_t)) <= 0) {
    if (secureRecv(clients[clId].fd,(ioBuffer+1), 
            2 * sizeof(int32_t) + tempSymtable[ioBuffer[0]].length * sizeof(int32_t)) <= 0) 
    {
      closeClient(clId);
      return -1;
    }
...
//      v = ntohl(ioBuffer[iterator2 + sizeof(struct timeval)/sizeof(int32_t) + 1]);
      v = ntohl(ioBuffer[iterator2 + 2 + 1]);
      *d = v;
//       if (v != *d)
//       { // value is changed
//         *d = v;
