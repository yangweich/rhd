...
int recieveSymboltable(char dir) {
  ...
  for(i = 0; (i < tempTableSize); i++) {
    ...
    // timestamp is 2 32 bit integers (also on 64 bit machines)
  ...
  for(i = 0, tmpPtr = tempDatapool; (i < tempTableSize); i ++) {
    // 32bit integer 2 and 3 are no longer direct a "struct timeval", 
    // but 32bit values of seconds and micro-seconds - as on 32 bit machines
    tempTable[i].timestamp = &tempTable[i].time;
    tmpPtr += 2; // sizeof(struct timeval)/sizeof(int32_t);
...
int recieveDatapackage (char dir) {
  ...
  for(; pkgSize > 0; pkgSize--) {
      //Read variable id
      int nb = sizeof(int32_t);
      int32_t * pio;
      int idx;
      symTableElement * ste;
      if (secureRead(rhdSocket,ioData, nb) <= 0) {
        return -1;
      }
      ioData[0] = ntohl(ioData[0]); //Convert to host order
      idx = ioData[0];
      ste = &tempTable[idx];
      //nb = sizeof(struct timeval) + (ste->length * sizeof(int32_t));
      nb = (2 + ste->length) * sizeof(int32_t);
      //pio = ioData+1;
      pio = &ioData[1];
      //Read variable data
      if (secureRead(rhdSocket, pio, nb) <= 0) {
        return -1;
      }
      //Convert and write timestamp
      tempTable[idx].timestamp->tv_sec = ntohl(ioData[1]);
      tempTable[idx].timestamp->tv_usec = ntohl(ioData[2]);
      ...
      if (dir == 'r') 
        ...
        // Mark write variable as changed if so
  ...
  for(i = 0; i < wTableLen; i++) {
    if (wSymTable[i].updated >= 1) {
      *ioPtr = i; //Save index
      ...
      ioPtr[0] = wSymTable[i].timestamp->tv_sec;
      ioPtr[1] = wSymTable[i].timestamp->tv_usec;
      ioPtr += 2; //sizeof(struct timeval) / sizeof(int32_t);
      //Copy data
 ...
 * \returns int* array
 */
  int * readArrayNamed(char *arrayName) {
