...
{  
  ...
  interval = wf->ReadInt( wf_entity, "interval",  interval );

  interval = interval * 1000; //Convert from msec to usec
...
{     
	...
	//Only sample the range scanner at the defined interval
	if (world->SimTimeNow() > nextSampleTime) {
	  ...
	  //Move the next sampletime one element of interval forward
	  nextSampleTime += interval;

	} else { //No laser update is done

		updates--; //Decrement update counter if no laser update is done

	}

	//Call update from parent model
	Model::Update();
}
