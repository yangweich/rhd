...
/*********************** Version control information ***********************/
 #define REVISION         "$Rev: 479 $:"
 #define DATE             "$Date: 2014-04-15 12:24:08 +0200 (Tue, 15 Apr 2014) $:"
 #define ID               "$Id: fielddrive.c 479 2014-04-15 10:24:08Z jcan $"
...
//   char cmdStr[MAX_SPEED_STR_CNT];
  ...
  { // connect to joystick - if any
    ...
    sast.varEmergStopPushed = getDatabaseVariable('r', "emergencyswitch");
    printf(PLUGINNAME " emergency switch   (r) variable index=%d\n", sast.varEmergStopPushed);
  ...
  if ((!isMasterAlive && !sast.joyOverride) ||
      (getReadVariable(sast.varEmergStopPushed, 0) > 0))
...
{ // create variables in RHD
  sast.varEncLeft = createVariable('r',1,"encl"); // encoder value rear wheels
      ...
      case 3:
        ...
        { // get enable bit and device name
          ...
          for(i = 0; attr[i]; i+=2)
          {
            ...
            else if (strcmp("baudrate1",attr[i]) == 0)
              // motor interface speed
              busif1.baudrate = strtol(attr[i+1], NULL, 0);
...
{// steering and front encoders
  int result = 0;
  //
  result = dxl_initialize_usb(busif1.serialDev, busif1.baudrate);
...
///  control thread - for servo steering
...
{ // run in thread (steer)
  ...
  { // maintain interface
    ...
    if (sast.setNewPos)
    {
      sast.setNewPos = 0;
      // move servo
      changeGoalPos(sast.servos[0].servo, sast.servos[0].positionRef);
      changeGoalPos(sast.servos[1].servo, sast.servos[1].positionRef);
      // tell rhd
      setVariable(sast.varServoIntRef, 0, sast.servos[0].positionRef);
      setVariable(sast.varServoIntRef, 1, sast.servos[1].positionRef);
    }
    ...
    switch (m)
    {
      ...
      case 6: // temp
        servoIdx = 0;
        dxl = &sast.servos[servoIdx];
        n1 = getCurrentTemperature(dxl->servo);
        setVariable(sast.varServoTemp, servoIdx, n1);
        n1 = dxl_get_rxpacket_error_byte();
        setVariable(sast.varDynamixelErr, servoIdx, n1);
        m++;
        break;
      case 7: // temp
        servoIdx = 1;
      ...
      case 8: // volt
        servoIdx = 0;
        dxl = &sast.servos[servoIdx];
        n1 = getCurrentVoltage(dxl->servo);
    ...
    n1 = dxl_get_result();
    setVariable(sast.varDynamixelErr, servoIdx + 2, n1);
    setVariable(sast.varDynamixelErr, 4, m);
    if (a2 == 1)
    {
      float sr, sl, saddeg;
...
//           sast.servos[1].degreeRef - angleRight)/2.0;
  ...
  //
  // printf(PLUGINNAME ": servo pos %d, %d\n", p0, p1);
  // implement
  ...
  { // servo position needs to be updated
    ...
    sast.setNewPos = 1;
//     changePosL(sast.servos[0].servo, p0);
//     changePosL(sast.servos[0].servo, p1);
//     snprintf(s, 32, "K=%d,%d\n", p0, p1);
//     secure3Write(busif1.ttyDev, s);
...
//     setVariable(sast.varServoIntRef, 1, p1);
    ...
    {
      gettimeofday(&logtime, NULL);
      fprintf(freLog, "%lu.%06lu if1 new pos %d %d", logtime.tv_sec, logtime.tv_usec, p0, p1);
    }
    // end debug
  }
  return isOK;
