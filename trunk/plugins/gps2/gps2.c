 ...
 * 
 * Modified by Adriana Sidea, 2014-05-28, lines with AGCO
 */
 /***************************************************************************
...
int initGps(void) {
  ...
  //Create variables
  iConnect      = createVariable('r', 1, "GPSconnected");					//AGCO (verifies if the GPS is connected )
...
int parseNMEA(char* inBuf) {
  ...
  isOK = validateNMEA(inBuf);
    ...
    if(strncmp(inBuf,"$GNRMC", 6) == 0 || strncmp(inBuf,"$GPRMC", 6) == 0) {				//AGCO 
    ...
    //GPS Fix data
    else if(strncmp(inBuf,"$GNGGA", 6) == 0 || strncmp(inBuf,"$GPGGA", 6) == 0) {			//AGCO 
    ...
    else if(strncmp(inBuf,"$GNGLL", 6) == 0 || strncmp(inBuf,"$GPGLL", 6) == 0) {			//AGCO
    }
    //GNSS Dillution Of Presition and Active Satellites - Not implemented
    else if(strncmp(inBuf,"$GNGSA", 6) == 0 || strncmp(inBuf,"$GPGSA", 6) == 0) {			//AGCO	
    }
    //GNSS Satellites in view - Not implemented
    else if(strncmp(inBuf,"$GPGSV", 6) == 0) {
    }
    //GNSS Satellites in view - Not implemented  - GL: GLONASS talker ID				//AGCO
    else if(strncmp(inBuf,"$GLGSV", 6) == 0) {
    }
    //Course over Ground and Ground Speed
    //Redundant information - Not implemented
    else if(strncmp(inBuf,"$GNVTG", 6) == 0 || strncmp(inBuf,"$GPVTG", 6) == 0) {			//AGCO
    }  
...
// $PTNL,PJK,123148.30,091807,+6174357.366,N,+707854.368,E,1,07,3.1,EHT+64.037,M*74
  ...
  if (p1 != NULL)
    ...
    *p1 = '\0';
  setVariable(iConnect,0,(int)gpsRunning);							//AGCO
...
{ // $GPGGA,180432.00,4027.027912,N,08704.857070, W,2,07,1.0,212.15,M,-33.81,M,4.2,0555*73
  ...
  while (*p2 >= ' ')
  {
    ...
    { // decode parameters
      case 0: // just $GPGGA
        if (strcmp(p1, "$GNGGA") != 0 && strcmp(p1, "$GPGGA") != 0)					//AGCO
        {
  ...
  { // get string to next comma
    ...
    { // decode message parameters
      case 0: // just the name
        if (strcmp(p1, "$GNRMC") != 0 && strcmp(p1, "$GPRMC") != 0)					//AGCO
        {
