...
int getPressentVoltage(int Id)
{
	...
	#ifdef DEBUG
	printf("This Function will give you information on the current voltage supply");
	#endif 
	int CurrentVolt;
	CurrentVolt = dxl_read_byte(Id,42)/10;
	#ifdef DEBUG
	printf("Voltage = %d V\n",CurrentVolt);
...
int getPressentTemperature(int Id)
{
	...
	#ifdef DEBUG
	printf("This Function will give you information on the current Temperature");
	#endif 
	int CurrentTemp;
...
{
	#ifdef DEBUG
	printf("This Function will let you set goalposition\nNotice that it cannot cross 360 degrees.\n");
	printf("Therefore 361 does not exist, put 1, and it will go all the way back.");
	#endif 
	if(Id==254){
	  Id = BROADCAST_ID;
